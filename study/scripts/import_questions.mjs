...
fs.writeFileSync(DATA_JS, `// Generated by scripts/import_questions.mjs — do not edit. ${new Date().toISOString().slice(0, 10)}\nexport const PARTS = ${JSON.stringify(parts)};\n`);
// The outline itself (code, title, path) for the follow endpoints, so a follow can be
// validated and an alert can name the page without the Functions reading the site.
const OUTLINE_JS = new URL('../functions/_lib/outline.data.js', import.meta.url).pathname;
const outline = [...position.entries()].map(([code, p]) => ({ code, part: p.part, title: p.pageTitle, path: p.pagePath }));
